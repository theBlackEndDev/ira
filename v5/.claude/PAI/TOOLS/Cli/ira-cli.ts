...
function cmdTmuxStart(args: string[]) {
  ...
  if (!binaryInstalled(primaryTarget)) {
    die(
      ...
      `Install Claude Code, or check primary_target in ~/.config/ira/config.jsonc.`
    );
  }
...
function cmdTeam(args: string[]) {
  ...
  // Claude Code per-pane command (--prompt). A future Antigravity adapter wires its own
  // invocation here via the target seam (see PAI/TOOLS/targets/TARGETS.md).
  const escapedPrompt = `[${agent}] ${prompt.replace(/"/g, '\\"')}`;
  const binaryCmd = `${primaryTarget} --prompt "${escapedPrompt}"`;
...
Session Management:
  ira tmux start [name] [options]      Create a new IRA tmux session
                                        Options:
                                          ...
                                          --target claude        (IRA is Claude-only; Antigravity planned)
